																...
																<span onclick="uploadQr();" class="profile-picture">
																	<c:choose>
																	   <c:when test="${list == null || list.size() < 1 }">
																	   		<img id="voucher_qr" class="editable img-responsive" src="${base}static/images/addFile.jpg"/>
																	   </c:when>
																	   <c:otherwise> 
																			<c:forEach items="${list}" var="qr" varStatus="vs">
																				<img id="voucher_qr" class="editable img-responsive" src="${base}manage/img/showByPath.do?path=${qr.path}"/>
																			</c:forEach>	
																	   </c:otherwise>
																	</c:choose>
																</span>
...
$.UploadConfig.onchange("#uploadFile", "#voucher_qr");
</script>
